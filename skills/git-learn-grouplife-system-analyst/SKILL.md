...
## Phase 4.5 — เลือกโหมดการรัน: Break point ทีละเมนู / รันรวดเดียว (เพิ่ม 2026-09-25 ตามฟีดแบ็กผู้ใช้)

เหตุผล: วิเคราะห์หลายเมนูในรอบเดียวกิน token สูงมาก เสี่ยง token ในเครื่องหมดกลางคัน
ผู้ใช้จึงต้องการเลือกได้ว่าจะหยุดพักทุกครั้งที่จบ 1 เมนูหรือไม่

**ถ้าผลลัพธ์ Phase 4 มี leaf menu ที่เลือก ≥ 2 เมนู** ต้องเรียก `AskUserQuestion` **จริง**
(ห้ามข้าม ห้ามเดาเอง แม้ผู้ใช้เคยเลือกโหมดไว้ในรอบรันก่อน ๆ — ถามใหม่ทุกครั้งที่เรียก skill)
1 คำถาม `multiSelect: false`, `header: "โหมดรัน"` พร้อม 2 ตัวเลือก:
1. **"Break point ทีละเมนู (Recommended)"** — description: "วิเคราะห์+เขียน Sheet ทีละเมนู
   จบ 1 เมนูแล้วหยุดรอสั่ง continue ก่อนเริ่มเมนูถัดไป — ประหยัด token/คุมได้ ไม่เสียงานถ้า
   token หมดกลางทาง"
2. **"รันทุกเมนูรวดเดียว"** — description: "วิเคราะห์ทุกเมนูที่เลือกต่อเนื่องโดยไม่หยุด
   (Agent ขนาน) — เร็วกว่า แต่ใช้ token มากในรอบเดียว"

ถ้าเลือกไว้แค่ 1 เมนู ไม่ต้องถาม — ทำ Phase 5 → 6 ตามปกติ

### โหมด A: Break point ทีละเมนู

- ทำ `drive_ops.py ensure` (Phase 6) **ครั้งเดียว** ก่อนเริ่มเมนูแรก เก็บ `spreadsheet_id` ไว้
- จากนั้นวน**ทีละเมนูตามลำดับที่เลือก** (ห้ามขนาน): เรียก Agent **1 ตัว**ของเมนูนั้น
  (prompt เหมือน Phase 5 ทุกประการ) → รับ JSON → เขียนด้วย `sheet_write.py` ทันที (Phase 6
  รวมถึงกฎ stamp WinExec ถ้าเข้าข่าย) → **จบ turn** ด้วยสรุปสั้น ๆ:
  - `[เมนูที่ n/ทั้งหมด N] <breadcrumb>` ✅ เสร็จ + `tab_url` + จุดที่ยังวิเคราะห์ไม่ครบ (ถ้ามี)
  - รายการเมนูที่**เหลือ** (breadcrumb เต็ม + ฟอร์ม candidate) พร้อม `spreadsheet_id`/Group/
    AppName/repo/sub_folder — เพื่อให้ resume ได้แม้ต้องเปิด session ใหม่เพราะ token หมด
  - บอกผู้ใช้ให้พิมพ์ `continue` เพื่อเริ่มเมนูถัดไป (หรือ `stop` เพื่อจบ)
- **ห้ามเริ่มเมนูถัดไปเอง**จนกว่าผู้ใช้จะสั่ง continue (หรือคำที่มีความหมายเดียวกัน เช่น
  "ต่อ", "ไปต่อ", "next") — ถ้าผู้ใช้สั่ง stop/หยุด ให้ข้ามไป Phase 7 สรุปเฉพาะเมนูที่เสร็จแล้ว
  และระบุรายการเมนูที่ยังค้างอยู่
- เมื่อครบทุกเมนูแล้ว ทำ Phase 7 สรุปรวมตามปกติ

### โหมด B: รันทุกเมนูรวดเดียว

ทำ Phase 5 (Agent ขนานในข้อความเดียว) → Phase 6 (เขียนทีละเมนูตามลำดับ) → Phase 7
ต่อเนื่องโดยไม่หยุดถาม — เหมือนพฤติกรรมเดิมของ skill

## Phase 5 — วิเคราะห์แบบขนาน (แยก Agent ต่อ 1 เมนูที่เลือก)

> ในโหมด A (Break point) ของ Phase 4.5 ให้เรียก Agent ทีละ 1 ตัวต่อรอบแทนการยิงขนาน —
> prompt/กฎด้านล่างทั้งหมดยังใช้เหมือนเดิม
