#!/usr/bin/env python3
"""Browse and read files from ANY Gitblit repo over the web UI, WITHOUT ever
running `git clone` — this is the whole point of this skill's sibling
relationship to /git-clone (same host/repo path, but this script reads
content remotely instead of materializing it on disk). Unlike its sibling
/git-learn-grouplife (which is hardcoded to the "delphi" group repos listed
in a tracking sheet), this script takes the full Gitblit repo path directly
from the caller and works against any repo on the same Gitblit instance.

Usage:
  browse_repo.py list <repo> <path> [--branch B]
      Lists the immediate children of <path> in <repo> (default branch
      "master"). <repo> is the full Gitblit repo path as it appears in the
      web UI / SSH clone URL, minus the "ssh://user@host:port/" prefix and
      the trailing ".git" (e.g. "delphi/groupwork-system-2016", or just
      "myrepo" if it has no group folder). Prints JSON: {"path": "...",
      "entries": [{"type": "folder"|"file", "name": "...", "path": "..."}]}.
      A folder's "path" already reflects any Gitblit auto-collapse (may
      contain "/").

  browse_repo.py tree <repo> <path> [--branch B] [--max-requests N]
  ...
  browse_repo.py read <repo> <path> [--branch B] [--max-bytes N]
...
def encoded_repo(repo):
    return urllib.parse.quote(f"{repo}.git", safe="")


def list_folder(session, repo, path, branch="master"):
    enc_repo = encoded_repo(repo)
    if path:
        url = f"tree/{enc_repo}/{branch}/{urllib.parse.quote(path, safe='')}"
    else:
        url = f"tree/{enc_repo}"
    ...
    for kind, href in ROW_RE.findall(html):
        ...
        # rest looks like "<enc_repo>/<branch>/<enc_path>" (root listing omits
        # "/<branch>/<path>" entirely when rest has no further slash)
        parts = rest.split("/", 2)
...
def tree(session, repo, start_path, branch="master", max_requests=500):
    ...
    while queue:
        if requests_used >= max_requests:
            ...
            break
        path = queue.popleft()
        entries = list_folder(session, repo, path, branch)
        requests_used += 1
        folders_scanned += 1
...
def read_file(session, repo, path, branch="master", max_bytes=2_000_000):
    enc_repo = encoded_repo(repo)
    enc_path = urllib.parse.quote(path, safe="")
    url = f"raw/{enc_repo}/{branch}/{enc_path}"
...
def main():
    ...
    try:
        ...
        if args.cmd == "list":
            result = {"path": args.path, "entries": list_folder(session, args.repo, args.path, args.branch)}
        elif args.cmd == "tree":
            result = tree(session, args.repo, args.path, args.branch, args.max_requests)
        elif args.cmd == "read":
            result = read_file(session, args.repo, args.path, args.branch, args.max_bytes)
        else:
